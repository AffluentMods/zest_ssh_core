...
-----END EC PRIVATE KEY-----''';
  ...
  group('OpenSSHBcryptKdfOptions.decode rounds bound', () {
    // Regression: 1.6.0's SSH-core hardening added a `rounds > 64` ceiling on
    // decode, which rejected every passphrase-protected key created with a
    // non-default KDF cost. `ssh-keygen -a 100` is common for hardened keys,
    // and those failed to load with
    // "OpenSSH key bcrypt rounds out of range: 100 (expected 1-64)".
    // The ceiling is now 1000. See docs/security-audit-2026-08.md.
    Uint8List encodeKdf(int rounds) =>
        OpenSSHBcryptKdfOptions(Uint8List(16), rounds).encode();

    test('accepts the ssh-keygen default of 16 rounds', () {
      expect(OpenSSHBcryptKdfOptions.decode(encodeKdf(16)).rounds, 16);
    });

    test('accepts hardened keys (ssh-keygen -a 100)', () {
      expect(OpenSSHBcryptKdfOptions.decode(encodeKdf(100)).rounds, 100);
    });

    test('accepts up to the 1000-round ceiling', () {
      expect(OpenSSHBcryptKdfOptions.decode(encodeKdf(1000)).rounds, 1000);
    });

    test('rejects a zero or DoS-sized round count', () {
      expect(() => OpenSSHBcryptKdfOptions.decode(encodeKdf(0)),
          throwsFormatException);
      expect(() => OpenSSHBcryptKdfOptions.decode(encodeKdf(1001)),
          throwsFormatException);
    });
  });
}
