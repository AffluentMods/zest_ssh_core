...
void main() {
  ...
  group('SSHTransport AEAD', () {
    test('exchanges packets with AES-GCM', () async {
      ...
      // This test injects cipher keys directly to exercise the encrypted
      // data path, skipping the real handshake. The transport (correctly)
      // refuses to deliver post-KEX application messages before the host key
      // is verified, so mark it verified to model the genuine
      // post-handshake state this test is standing in for.
      setPrivate(receiver, '_hostkeyVerified', true);
