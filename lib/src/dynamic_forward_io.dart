...
class _SocksConnection {
  ...
  /// Upper bound on bytes buffered before the handshake completes. A full
  /// SOCKS5 greeting plus CONNECT request is at most ~520 bytes; anything
  /// larger while still handshaking is malformed or hostile, so refuse it
  /// rather than let the pre-stream buffer grow without limit.
  static const _maxHandshakeBytes = 4096;
  ...
  Future<void> _onClientData(List<int> chunk) async {
    ...
    // Bound the pre-stream handshake buffer against a client that dribbles
    // bytes without ever completing a valid greeting/request.
